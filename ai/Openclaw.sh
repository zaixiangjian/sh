...
" "$status_color_fail" "$status_text" "$status_color_reset"
		...
		openclaw_model_probe() {
			...
			local oc_config provider_name base_url api_key request_model provider_api
			...
			provider_api=$(jq -r --arg provider "$provider_name" '.models.providers[$provider].api // empty' "$oc_config" 2>/dev/null)
...
req = urllib.request.Request(
    ...
    headers={
        ...
        'User-Agent': 'OpenClaw-Model-Probe/1.0',
        'Authorization': f'Bearer {api_key}',
    },
    method='POST',
)
...
PYTHON_EOF
			...
			case "$(printf '%s' "$reply_trimmed" | tr '[:upper:]' '[:lower:]')" in
				*"all available accounts exhausted"*|*"no available accounts"*)
					OPENCLAW_PROBE_MESSAGE="接口可达，但后端账号池无可用账号（${first_endpoint} HTTP ${first_http:-0}；${second_endpoint} HTTP ${second_http:-0}）"
					;;
				*"upstream access forbidden"*)
					OPENCLAW_PROBE_MESSAGE="接口可达，但上游访问被拒绝/未授权（${first_endpoint} HTTP ${first_http:-0}；${second_endpoint} HTTP ${second_http:-0}）"
					;;
				*"error code: 1010"*)
					OPENCLAW_PROBE_MESSAGE="检测请求被防火墙/WAF 拦截（${first_endpoint} HTTP ${first_http:-0}；${second_endpoint} HTTP ${second_http:-0}），不等于模型名错误"
					;;
				*)
