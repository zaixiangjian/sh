...
crontab -l 2>/dev/null | grep -E "beifen.x|chuansong.x|zidongtuchuang.x|wangpan.x|zixuanmulu.x" | nl | while read -r line; do
  ...
  if echo "$content" | grep -q "beifen.x"; then
    ...
    echo "$num. 其他任务: $content"
      ...
      echo "99. 网站备份全部传送添加路径https://github.com/zaixiangjian/sh/blob/main/quanbubeifei.sh"
...
EOF
  ...
  99)
    echo "开始创建 99 号监控备份任务..."
    echo "/home/博客 /home/图床 /home/密码 /home/论坛"


    BACKUP_SOURCE="/home/博客 /home/图床 /home/密码 /home/论坛"
    ...
    # ------------------ 生成原始脚本 ------------------
    TMP_SCRIPT="/home/docker/wangpan13_tmp.sh"
    OBFUSCATED_SCRIPT="/home/docker/wangpan13_obf.sh"
    OUTPUT_BIN="/home/docker/wangpan13.x"
...
tar -czf "$BACKUP_OUTPUT" $BACKUP_SOURCE
EOF
    ...
    rm -f "$TMP_SCRIPT" "$OBFUSCATED_SCRIPT"
