...
OFFICIAL_IMAGE="gmwallet/epusdt:latest"
CUSTOM_IMAGE="zaixiangjian/epusdt:latest"
...
show_status() {
  echo
  msg "${BLUE}GMPay / Epusdt 管理脚本${PLAIN}"
  echo "安装目录：${APP_DIR}"
...
install_app() {
  ...
  if [ -f "${APP_DIR}/${COMPOSE_FILE}" ]; then
    warn "检测到 ${APP_DIR} 已存在，安装操作将保留现有 data 并重建/启动服务。"
  ...
  success "GMPay / Epusdt 安装/启动完成"
  echo
  echo "访问地址：http://服务器IP:${port}"
  echo "安装目录：${APP_DIR}"
  echo "配置文件：${APP_DIR}/data/.env"
  echo "重要：Docker 安装向导里的 http_bind_addr 请设置为 0.0.0.0，不要用 127.0.0.1。"
}
...
backup_app() {
  ...
  if is_stack_running; then
    was_running=1
    warn "检测到容器运行中，先停止服务以保证 SQLite/运行时数据备份一致性..."
    stop_stack
  fi
...
uninstall_app() {
  require_root
  warn "此操作将停止并删除 GMPay / Epusdt 容器与安装目录：${APP_DIR}"
...
docker_push_app() {
  ...
  if ! docker image inspect "${CUSTOM_IMAGE}" >/dev/null 2>&1; then
    if docker image inspect "${OFFICIAL_IMAGE}" >/dev/null 2>&1; then
      warn "本地没有 ${CUSTOM_IMAGE}，将 ${OFFICIAL_IMAGE} 标记为 ${CUSTOM_IMAGE} 后推送..."
      docker tag "${OFFICIAL_IMAGE}" "${CUSTOM_IMAGE}"
    else
      warn "本地没有 ${CUSTOM_IMAGE}，尝试先拉取镜像..."
...
main_menu() {
  while true; do
    ...
    show_status
    echo "1. 安装（zaixiangjian/epusdt这里就使用官方容器名一致）"
    echo "2. 更新（zaixiangjian/epusdt这里就使用官方容器名一致）"
    echo "3. 备份（/home/${BACKUP_PREFIX}-YYYYmmddHHMMSS.tar.gz）"
    echo "4. 恢复（从 /home/${BACKUP_PREFIX}-*.tar.gz 获取，回车默认最新）"
    echo "5. 登录docker"
    echo "6. 推送到docker（zaixiangjian/epusdt）"
    echo "7. 卸载"
    echo "------------------------------------------------"
    echo
    echo "11. 安装"
    ...
    case "${choice}" in
      1) install_app "${CUSTOM_IMAGE}"; pause ;;
      2) update_app "${CUSTOM_IMAGE}"; pause ;;
      3) backup_app; pause ;;
