...
kj_app_add_domain() {
	...
	wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}https://raw.githubusercontent.com/zaixiangjian/nginx/main/reverse-proxy.conf
	sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	sed -i "s/0.0.0.0/127.0.0.1/g" /home/web/conf.d/$yuming.conf
	sed -i "s/0000/$port/g" /home/web/conf.d/$yuming.conf
	docker restart nginx >/dev/null 2>&1 || docker exec nginx nginx -s reload >/dev/null 2>&1
	echo "已添加域名访问: https://$yuming -> 127.0.0.1:$port"
}
